...
#这个脚本用来实现MoptL模块下的数据驱动的多目标优化功能。采用了BP神经网络
#
#**********************************************************************************************************************************
import numpy as np
import torch
...
class BPNeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        hidden = self.hidden_activation(self.hidden_layer(x))
...
def preprocess_data(data, n_features):
    ...
    # BP神经网络训练函数
def train_bp_network(X_train, y_train, input_size, hidden_size, output_size, learning_rate=0.01, epochs=1000):
    """
    训练BP神经网络
...
def main(data, n_features, hidden_size=64, learning_rate=0.01, epochs=1000):
    ...
    # 训练模型
    model, losses = train_bp_network(X_train, y_train, input_size, hidden_size, output_size, learning_rate, epochs)
