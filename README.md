...
- 目前Beta 2.0.015 已经发布，尽管还有一些已知BUG,但我们不会继续发布新的修改版本，我计划发布年度更新2.1.000版本，该版本包含如下的重大功能改进和实现：

##### 1）全新TurbineL模块
- 该模块支持气动功与气动阻尼的计算和输出，以评判叶片的失稳分险，探讨叶片在TMDI作用下的不稳定现象，揭示增稳机理。主要是针对大论文的创新点3提供计算工具。
- 气弹耦合将从MBD多体动力学当中独立出来并耦合到TurbineL当中，以实现独立的刚性/柔性气弹计算，包含刚性/柔性Cp，包含刚性/柔性气动功率曲线。
- TurbineL 支持计算风轮的坎贝尔图，并支持计算叶片模态和固有频率
- TurbineL 耦合了BeamL、FEML当中的有限元方法，基于浮动坐标法，实现MBD/FEM（叶片）的精确耦合。

##### 2）全新MSAL 模型线性化模块
- 该模块可以对整机模型进行线性化以分析结构动力学特征，并输出相关的参数，目前的理论学习尚未开展。

##### 3）可能支持
- 目前FVW 方法的代码已经开发，但没有并入到代码当中，主要是接口师傅存在冲突，我不确定是否可以在这个版本当中支持
- MBD 的双机头，尽管该功能开发较为简单，但涉及的代码量巨大，忙于毕业，可能不会发布。
